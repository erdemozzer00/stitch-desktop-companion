...
internal static class TrayVisualPrototype
{
    ...
    static void ActionIcon(Graphics g,string kind,float x,float y,Color ink)
    {
        ...
        using(var p=new GraphicsPath())
        {
            if(kind=="wave")
            {
                p.AddLines(new[]{new PointF(6,17),new PointF(2,10),new PointF(3,8),new PointF(6,11),new PointF(4,4),new PointF(6,3),new PointF(9,9),new PointF(7,1),new PointF(9,1),new PointF(12,8),new PointF(11,2),new PointF(13,3),new PointF(15,11)});
                p.AddBezier(15,11,17,16,13,21,8,19);g.DrawPath(pen,p);
                g.DrawArc(pen,12,-3,8,10,300,90);
            }
            else
            {
    ...
    static void Panel(Graphics g,float x,float y,bool hidden=false,bool focus=false)
    {
        var state=g.Save(); g.TranslateTransform(x,y);
        for(int i=14;i>0;i--)
            Box(g,-i/2f,4-i/3f,312+i,284+i/1.5f,16+i/3f,Color.FromArgb(3,0,0,0));
        using(var p=Round(.5f,.5f,311,283,16))
        using(var b=new LinearGradientBrush(new Rectangle(0,0,312,284),C(24,21,31),C(40,29,49),65f))
        ...
        Box(g,16,78,280,46,9,hidden?C(42,35,51):C(63,47,79));
        ActionIcon(g,"wave",32,90,hidden?Muted:White);
        Text(g,"El salla",62,91,15,hidden?Muted:White);
        if(focus)using(var p=Round(18,80,276,42,8))using(var pen=new Pen(C(197,178,227),1.5f))g.DrawPath(pen,p);
        Text(g,"Boyut",20,144,12,Muted);
        Box(g,16,166,280,38,8,C(20,17,26));
        Box(g,19,169,90,32,6,C(66,54,79));
        Center(g,"Küçük",new RectangleF(19,169,90,32),13,White);
        Center(g,"Orta",new RectangleF(111,169,90,32),13,Muted);
        Center(g,"Büyük",new RectangleF(203,169,90,32),13,Muted);
        Line(g,Color.FromArgb(24,230,222,247),20,224,292,224);
        ActionIcon(g,hidden?"show":"hide",24,243,White);
        Text(g,hidden?"Göster":"Gizle",51,243,13,White);
        Glyph(g,"\uE7E8",225,245,16,Muted);Text(g,"Çıkış",249,243,13,Muted);
        g.Restore(state);
    }
    ...
    static void Main(string[] args)
    {
        ...
        using(stitch=Image.FromFile(args[0]))
        {
            ...
            Graphics g;using(var b=Canvas(1408,732,out g))using(g)
            {
                g.Clear(C(18,18,24));g.ScaleTransform(2,2);
                Text(g,"GÖRÜNÜR",26,16,10,Muted);Text(g,"GİZLİ",374,16,10,Muted);
                Panel(g,22,48,false,false);Panel(g,370,48,true,false);
