...
sources = {}
for clip, count in [('idle', 96), ('wave', 45), ('entries', 64)]:
    folder = phase / clip
    ...
    for name, digest in hashes.items():
        ...
        sources[name] = path
    if clip == 'entries':
        assert manifest['buckets'] == 16 and manifest['frames_per_entry'] == 4
...
for name in ['idle_0001.png', 'wave_0045.png'] + ['entry_%02d_0004.png' % i for i in range(16)]:
    with Image.open(sources[name]) as image:
        assert np.array_equal(neutral, np.asarray(image)), ('Neutral mismatch', name)
for bucket in range(16):
    with Image.open(sources['entry_%02d_0001.png' % bucket]) as entry, Image.open(sources['idle_%04d.png' % (bucket * 6 + 1)]) as idle:
...
report = {'status': 'PASS', 'revision': 'phase03-v3', 'frames': len(sources),
