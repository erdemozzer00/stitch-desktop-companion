...
## Current personal delivery

`Desktop/Stitch - Hediye` is the active private standalone folder. Desktop `Stitch.lnk` and the current-user Startup shortcut target its executable. Setup is `Kur.cmd` (delegates to Kur.ps1); Turkish usage/removal instructions are included. Windows sign-in startup is enabled, but an actual reboot/logon has not been performed. The application automatically loads its adjacent carry folder and enforces one instance per current user/session. Reopening restores the existing pet without opening the remote.

Reproduce a new clean package using `scripts/package_personal.ps1 -Destination <new-private-folder>` after building/staging the checked candidate and icon. The package contains proprietary rendered assets and must not be pushed to the public repository. The old trial updater does not update an already-created delivery folder. See context/ACTIVE.md before future deployment.
