...
Recorded 2026-09-19. The user accepts the refined idle and final 45-frame wave. The 400px appearance candidate is rendered, checked and installed. See [execution evidence](evidence/phase-03-review.md). Final user appearance review remains open.
...
Current approved refinement: increase existing idle chest motion by 25% and head motion by 20%, anchored to the same neutral; retain timing, ears/arms and the accepted wave. Rebuild entry transitions and synchronize the live app. Then return to Step 03.4 dark-background contrast, outline and true 400px quality. No added gestures or Phase 04 carry work. The prior research-only pause is superseded by explicit user approval.

Latest update: the user likes the installed idle polish. It is now accepted; preserve motion. Step 03.4 selected soft lower fill after four appearance comparisons; full 400px/24-sample assets under `appearance-final` have passed checks and are installed. Legacy outline stays disabled after its dark comparison output. Await final user appearance feedback for the Phase 03 visual handoff.
