...
**Resume:** native open-eye carry was accepted; the installed native-trial has 400px carry and earlier idle handoff with response parameters preserved. The user approved the compact detached tray remote (no wave command) and requested integration. It is now checked and installed: startup panel closed, tray click opens size/hide-show/exit controls. Physical user review is next; see ACTIVE.md and [integration evidence](evidence/phase-04-tray-integration.md). Motion/controller/assets are unchanged by this UI step. Release-polish user feedback remains open. Do not re-ask for design approval, accepted directional motion or cancelled eyes. The original Phase 03 installation and frozen desktop fallback remain intact.

Current handoff supersedes the historical Resume paragraph above: user requested Phase 05 continuation, now implemented as minimal personal delivery. See ACTIVE.md; no animation changes in this phase.
