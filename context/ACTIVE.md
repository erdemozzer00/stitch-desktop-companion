...
**Current cursor:** The user accepted actual open-eye carry ("10/10") and authorized release-quality/pause fixes plus a dark-purple gradient/white-text UI. Implemented and installed to the SAME `.local/phase-04/native-trial/StitchPet.exe` / desktop `Stitch - Tasima Denemesi` shortcut: 400px/24-sample carry, byte-identical neutral endpoint, earlier visible idle handoff with residual settling preserved, hover command bar and styled right-click/tray menu. Debug panel is opt-in `-Probe`, not the default. Latest executable hash/backup: `phase-04-polished-install.json`; launch PID 27368 may be stale. New user visual/UI review is pending; do not repeat the accepted directional experiment or change drag strength. See latest Phase 04 evidence. Eyes stay OPEN; no Godot or Phase 05 startup work.
