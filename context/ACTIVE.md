...
**Current cursor:** The user accepted directional-v1 reaction strength and requested eyes closed while carried. The private geometric-eyelid refinement is ready: `.local/phase-04/directional-eyes-v1/carry-eyes-light.gif` and dark counterpart. Left is accepted open-eye response; right adds closing while held and reopening after release. Controller and all 205 pointer/pose records match the accepted replay exactly. See `phase-04-carry-eyes-checks.json` and [latest execution evidence](evidence/phase-04-review.md). Facial refinement awaits user moving review; next is the isolated actual-pointer host trial. Native input, arbitrary moving eye transitions and wave/idle entry integration remain unimplemented. Do not repeat the old pickup-only study.
