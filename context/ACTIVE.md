...
**Current cursor:** The user accepted directional-v1 reaction strength and subsequently CANCELLED the closed-eye addition. Use the original open-eye `.local/phase-04/directional-v1/drag-bank.blend` and its 45-pose bank. Preserve its controller and strength. Next is Step 04.2: the isolated actual-pointer host trial, including wave/idle entry and interruption handling. No eye review or eye-transition work remains required. The private directional-eyes-v1 artifacts and eye scripts are historical experiments only; do not integrate them. See [latest execution evidence](evidence/phase-04-review.md). Do not repeat the old pickup-only study.
