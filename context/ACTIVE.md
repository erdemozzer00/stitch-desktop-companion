...
**Current cursor:** Step 04.2 is implemented as an isolated open-eye native trial at `.local/phase-04/native-trial/StitchPet.exe`, enabled only with `--carry=<trial>/carry`. Use `scripts/start_carry_trial.ps1 -DesktopShortcut` after checks. The user must now assess actual pointer response, off-center grips and transitions. The controller matches the accepted Python replay; preserve its strength. Idle uses the existing four-frame entry to neutral; an active wave finishes while moving/rotating before the articulated carry bank takes over. This compromise requires user review. Check `phase-04-native-launch.json` and real process/log before claiming it is still running. See [latest execution evidence](evidence/phase-04-review.md). Eyes stay OPEN; cancelled eye assets must not be integrated.
