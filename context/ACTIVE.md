...
2026-09-24 recipient setup blocker: Kur.ps1 rejected an existing Startup Stitch.lnk targeting the previous gift folder. Fixed `setup_personal_package.ps1` and installed matching `Hediye/Kur.ps1`: recognize prior installer-owned links by executable name, Description, empty arguments and matching working directory; back up and retarget them after checking both destinations. Unrelated links remain protected. Actual Windows PowerShell/COM isolated shortcut checks passed; recipient retry pending. Send `Desktop/Stitch-Kurulum-Duzeltmesi.zip`, replace only Kur.ps1 inside the recipient's Hediye folder, close the old Stitch, then run existing Kur.cmd. No need to resend model assets or reinstall Blender. See [setup repair](evidence/setup-shortcut-migration.md). Do not delete arbitrary Startup contents or change this PC's shortcuts to reproduce the issue.
