...
**Current cursor:** The user APPROVED the compact tray prototype and explicitly authorized integration. Implemented, checked and installed `phase04-tray-remote` to the SAME `.local/phase-04/native-trial/StitchPet.exe` / desktop `Stitch - Tasima Denemesi` shortcut. Character-attached controls removed; launch shows pet and Stitch tray icon with panel CLOSED. Tray left-click opens size/hide-show/exit remote; no El salla command. Native-form checks PASS; physical tray interaction/user review is next. See [integration evidence](evidence/phase-04-tray-integration.md). PID 41748 may be stale; latest exe hash/backup in `phase-04-polished-install.json`. The prior release fix still needs explicit user feedback. Preserve accepted carry strength and OPEN eyes; no Godot or Phase 05 startup work.
...
Latest UI: approved 312 x 220 logical-pixel tray remote is integrated. El salla is absent from both remote and tray fallback; waving by clicking Stitch is preserved. No hover/character-local control panel. Await physical tray/user acceptance, not another prototype approval.
