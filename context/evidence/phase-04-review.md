...
## User acceptance, release polish and contextual UI — 2026-09-19

The user accepted the native carry behavior overall ("10/10"), reported a short pixelated/near-static release interval, and authorized fixing it before adding simple recipient-friendly controls. The final UI direction explicitly changed from black/white to a subdued near-black purple gradient and white text. No change to accepted carry strength, no eye closure and no Godot.

### Diagnosis and correction

Read-only analysis found two distinct mechanisms: carry images were 240px/six samples while idle was 400px/24 samples; and idle was gated on a very small response cutoff even after the bank was visually neutral. A controlled diagonal release at 60/120Hz reached neutral/sub-quarter-pixel global motion around 0.39s but did not resume idle until 0.717s, leaving about 0.33s nearly still. Real event logs contained 33 uninterrupted release/settle pairs, mean about 0.617s (not video proof of a frozen UI thread).

`render_carry_final.py` loads the accepted open-eye bank, preserves its action/geometry/camera/source bytes and renders 44 poses at 400px/24 samples. The 45th, neutral image reuses the accepted idle endpoint bytes. No new animation combinations were added. All 45 hashes, dimensions and alpha margins are validated before staging. Bank PNGs occupy about 4.04 MiB; its loaded raw pixels are 27.47 MiB. The complete runtime loads 282 frames (233 unique PNG hashes, but duplicates are currently separately decoded), about 172.12 MiB raw pixels. This is below the proposed 96-added-frame cap and is not total process memory.

`CanResumeIdle(size)` allows idle once the bank is neutral and residual rotation is below a quarter displayed pixel, after release. It also requires low angular and bank-coordinate speed so a fast transient crossing of neutral is not mistaken for rest. Residual spring integration continues independently. The host no longer waits for numerical rest before breathing starts, and the later numerical completion does not reset idle again. Existing body/ear/vertical response parameters are unchanged; click reaction becomes available once the visible carry handoff is complete.

### UI research and implementation

Context7 queried official Windows Forms guidance on custom ToolStrip renderers, gradients, button states and accessibility. Design references: [Microsoft contextual menus](https://learn.microsoft.com/en-us/windows/apps/develop/ui/controls/menus-and-context-menus), [contextual commanding](https://learn.microsoft.com/en-us/windows/apps/develop/ui/controls/collection-commanding), and [accessible text contrast](https://learn.microsoft.com/en-us/windows/apps/design/accessibility/accessible-text-requirements). We use the small primary-command/secondary-menu pattern in the user's reference; no framework migration or copied product features.

`CompanionUi.cs` implements a rounded command bar with a restrained `#1F182C` to `#30203D` gradient, off-white `#F8F5FC` text and a quieter hover surface. It contains the labeled El salla button and an accessible more button with a tooltip. A native styled menu exposes wave, size submenu with current-size check, hide/show and exit. Hidden-state restoration remains available through the tray menu. No startup setting is displayed before Phase 05 implements it.

Controls appear on character hover and briefly at launch/restore, hide on grabbing, and retain a pointer corridor between character/bar/menu. Work-area placement flips the bar above the pet when the bottom edge has no room. Automatic showing uses ShowWithoutActivation; explicit commands can receive focus. Native buttons and menus provide the underlying keyboard mechanisms, with Escape on the bar. Physical keyboard, hover, screen-reader and multi-DPI acceptance are still untested. The local rendered bar scales with system DPI; app manifest remains system-DPI-aware, not a claim of per-monitor-DPI completeness.

The diagnostic probe no longer opens in normal trial launches; `start_carry_trial.ps1 -Probe` retains it for explicit diagnostics. Production startup/packaging remains deferred. Actual control renders are private at `.local/phase-04/polished-trial/ui-review/{toolbar,menu}.png` and were inspected. They are actual WinForms renders, not a replacement mockup or desktop screenshot.

### Verification and deployment

- `phase-04-polish-checks.json`: 96 assertions PASS for release handoff, contrast, negative-coordinate work-area placement, accessible name, real-button wave invocation, more-menu opening, size selection, toolbar hiding on grab/hide and menu restoration. Minimum tested enabled/muted palette contrast is 6:1. Disabled native menu text is not included in this contrast claim. With the final low-speed guard, sample 240px release resumes idle at 0.425s while a 0.292s residual tail continues; no fixed-delay shortcut was introduced.
- Baseline suite: 1,941 assertions PASS. Carry/state/pixel suite: 179,452 assertions PASS, mostly repeated border checks rather than distinct tests. Accepted Python response still matches within about 3.72e-8 radians. The 400px bank passed alpha/transform checks at 240/320/400 display sizes.
- Final direct-method Windows baseline/carry smoke passed at the new resolution. Carry process final Private Bytes about 202.47 MiB versus baseline 173.84 MiB (about +28.63 MiB); working set about 206.07 MiB. GDI objects stay at 29 in both short runs. Cumulative CPU is 7.313 seconds over 27.699 wall seconds versus baseline 2.141 over 27.663. This is more expensive during moving redraws than the 240px draft; it is not a whole-machine CPU percentage or a sustained performance guarantee. The smoke excludes the actual hover bar (`createTray=false`); the separate UI checks exercise controls but do not establish their long-run resource profile.
- Build treats warnings as errors; Python syntax and Git whitespace checks passed. Main Phase 03 executable and frozen fallback still have their original matching hash. No target Win11 claim.
- `update_carry_trial.ps1` verified candidate assets, gracefully closed only the exact prior trial's pet window, backed up its complete directory and preserved position/size at the SAME trial path. The original 240px trial is retained at `.local/phase-04/native-backup-20260919-171402-604`; the intermediate polish before the final speed guard is at `.local/phase-04/native-backup-20260919-172034-742`. It did not modify the original Phase 03 installation or desktop frozen fallback. The desktop shortcut launches without `--probe`.
